...
# 1. 페이지 레이아웃을 넓은 화면(Wide) 모드로 설정
st.set_page_config(layout="wide")

# 🟢 [핵심 장치] 버튼 클릭 시 다운로드 버튼이 증발하는 것을 막기 위한 세션 메모리 초기화
if "report_generated" not in st.session_state:
    st.session_state.report_generated = False
if "download_filename" not in st.session_state:
    st.session_state.download_filename = "Spectrum_Analysis_Report.pdf"

# 🎨 대시보드 UI 디자인을 위한 Custom CSS 주입
st.markdown("""
    <style>
    /* 입력창 라벨 디자인 깔끔하게 조정 */
    ...
    /* [가동 버튼] 주피터 특유의 success 연초록색 스타일 구현 */
    ...
    /* [다운로드 버튼] 신뢰감을 주는 학술 네이비 블루 스타일 구현 */
...
# 분석 모드 선택 라디오 버튼 (보내주신 사진 스펙 그대로 일치)
...
selected_option = st.radio("출력할 분석 모드 선택:", options=options, label_visibility="collapsed")
selected_mode = options.index(selected_option)

# 사용자가 입력을 바꾸면 기존 생성된 다운로드 버튼을 리셋하여 꼬임 방지
if st.sidebar.text(""): # 트리거용 더미 공간 확보 효과
    pass

# ==============================================================================
# [2. CORE ENGINE 컴파일러 구동 버튼 클릭부]
# ==============================================================================
if st.button("Spectrum Analysis Report 최종 컴파일 및 PDF 발행"):
    ...
    # 선택된 라디오 버튼 모드에 따른 가변 매개변수 분기 매핑
    if selected_mode == 0:
    ...
    # 3. 라디오 버튼 선택에 따라 동적으로 빌드되는 결과 페이지 분기
    story.append(PageBreak())
    story.append(Paragraph("3. 분석 방법론에 따른 맞춤형 분광 피팅 세부 결과", heading_style))

    if selected_mode == 0:
        story.append(Paragraph("<b>[선택 분석 모드: pPXF 연속광 공제 분석]</b> 활동은하핵(AGN) 고유의 가스 방출선을 분리하기 위해 Penalized Pixel-Fitting (pPXF) 최적화 알고리즘을 도입하여 모은하의 항성 기저 성분을 모델링한 후 차감하였습니다.", normal_style))
        ...
        story.append(Paragraph("<b>[선택 분석 모드: H-beta 방출선 성분 분해 분석]</b> 차감 완료된 순수 방출선 데이터로부터 광폭 방출선 영역(BLR)의 운동학 변수를 획득하기 위하여, H-beta 기저 영역에 대하여 비선형 최소제곱법 기반 다중 가우시안 성분 분해를 집행하였습니다.", normal_style))
        img_d = os.path.join("report_assets", "05_spectral_decomposition.png")
        if os.path.exists(img_d): story.append(Image(img_d, width=17.4 * cm, height=7.5 * cm, hAlign='CENTER'))
    else:
        story.append(Paragraph("<b>[선택 분석 모드: 비리얼 정리 블랙홀 질량 계산]</b> 분리된 Broad H-beta 방출선의 FWHM 속도 폭과 광도를 독립변수로 채택하여 Greene &amp; Ho (2005) 비리얼 스케일링 식을 적용하여 질량을 산출하였습니다.", normal_style))
        img_v = os.path.join("report_assets", "06_virial_broad_fit.png")
        if os.path.exists(img_v): story.append(Image(img_v, width=17.4 * cm, height=7.5 * cm, hAlign='CENTER'))
    ...
    interpret_txt = f"<b>[판독 결과]</b> 중심 블랙홀 질량 대표값은 태양 질량의 약 <b>{M_virial:,.0f} M_sun</b> 배이며 신뢰 범위는 <b>{M_lower:,.0f} M_sun</b> ~ <b>{M_upper:,.0f} M_sun</b> 입니다."
    story.append(Paragraph(interpret_txt, normal_style))

    # doc 파일 최종 조립 빌드 완료
    doc.build(story)

    # 🔥 [핵심 Fix] 연산 완료 상태와 최종 저장 파일명을 세션 메모리에 박제시킴
    st.session_state.report_generated = True
    st.session_state.download_filename = f"{target_name}_Analysis_Report_Mode_{selected_mode}.pdf"


# ==============================================================================
# [3. 다운로드 버튼 독립 렌더링 영역 - 무조건 노출 유지 구조]
# ==============================================================================
# 조건문 분기 바깥에 따로 꺼내져 있으므로 사용자가 다운로드를 눌러도 증발하지 않습니다.
if st.session_state.report_generated:
    if os.path.exists("Spectrum_Analysis_Report.pdf"):
        st.success("✓ 선택하신 분광 분석 보고서 컴파일이 완료되었습니다! 아래 버튼을 클릭하여 내려받으세요.")
        with open("Spectrum_Analysis_Report.pdf", "rb") as file:
            st.download_button(
                label="📥 생성된 분광 분석 PDF 보고서 최종본 다운로드",
                data=file,
